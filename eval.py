...
from utils import dense_crf, plot_img_mask, crop_v2_as_v1
# from utils import plot_img_mask, crop_v2_as_v1
...
def eval_net(net, dataset, gpu=False):
    ...
    for i, b in enumerate(dataset):
        ...
        y = crop_v2_as_v1(y_pred, y.unsqueeze(1))
        dice = dice_coeff(y_pred, y.float()).data[0]
        tot += dice

        if 0:
            X = crop_v2_as_v1(y_pred, X)
            X = X.data.squeeze(0).cpu().numpy()
            X = np.transpose(X, axes=[1, 2, 0])
            y = y.data.squeeze(0).squeeze(0).cpu().numpy()
